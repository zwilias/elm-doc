{- Largely copied from https://github.com/elm/package.elm-lang.org/blob/c5137ff4d18f7801ec4a59eb266cb72466b5efe9/src/frontend/Page/Docs/Block.elm

   See [license](https://github.com/elm/package.elm-lang.org/blob/master/LICENSE), copyright 2014-present Evan Czaplicki
-}


module Page.Block exposing
    ( Info
    , makeInfo
    , view
    )
...
view info block =
    case block of
        Docs.MarkdownBlock markdown ->
            span [ class "markdown-block" ] [ Markdown.toHtml [] markdown ]
...
viewCodeBlock name comment header =
    div [ class "docs-block", id name ]
        ...
        , div [ class "docs-comment" ] [ Markdown.toHtml [] comment ]
        ]
...
makeInfo : String -> V.Version -> String -> List Docs.Module -> Info
makeInfo author version moduleName docsList =
    let
        addUnion home union docs =
            Dict.insert (home ++ "." ++ union.name) ( home, union.name ) docs

        addAlias home alias_ docs =
            Dict.insert (home ++ "." ++ alias_.name) ( home, alias_.name ) docs

        addUnions docs dict =
            List.foldl (addUnion docs.name) dict docs.unions

        addAliases docs dict =
            List.foldl (addAlias docs.name) dict docs.aliases
        ...
        addModule docs dict =
            dict
                ...
                |> addAliases docs
    in
    Info author version moduleName <|
...
makeLink { name, version, moduleName } attrs tagName humanName =
    let
        url =
            "/" ++ name ++ "/" ++ V.toString version ++ "/" ++ moduleName ++ "#" ++ tagName
    in
